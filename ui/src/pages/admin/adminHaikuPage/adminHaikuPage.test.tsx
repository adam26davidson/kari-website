...
// Invokes the labeled option of the most recent confirmation dialog.
async function answerConfirmation(setConfirmation: Mock, label: string) {
  const confirmation = setConfirmation.mock.calls.at(-1)?.[0] as {
    options: Array<{ label: string; callback: () => void }>;
  };
  const option = confirmation.options.find((o) => o.label === label);
  await act(async () => {
    option?.callback();
  });
}
...
describe("AdminHaikuPage loading", () => {
  it("renders the loaded haiku list", async () => {
    ...
    expect(setLoading.mock.calls[0][0]).toEqual({
      isLoading: true,
      message: "Loading haiku...",
    });
    expect(setLoading.mock.calls.at(-1)?.[0]).toEqual({
      isLoading: false,
      message: "",
    });
  });
...
describe("AdminHaikuPage new haiku", () => {
  it("creates and opens a new empty haiku on Yes", async () => {
    ...
    await answerConfirmation(setConfirmation, "Yes");

    expect(HaikuService.updateList).toHaveBeenCalledWith(
      [first, second, { lines: [], publisher: "", id: "new-id" }],
      expect.any(Function),
    );
    expect(notify).toHaveBeenCalledWith("New haiku created");
  ...
  it("does nothing on No", async () => {
    ...
    await answerConfirmation(setConfirmation, "No");
...
describe("AdminHaikuPage deletion", () => {
  it("removes the confirmed haiku and saves the shortened list", async () => {
    ...
    expect(notify).toHaveBeenCalledWith("Haiku deleted");
  });
});
...
describe("AdminHaikuPage editing", () => {
  it("edits a copy and saves it back into the list by id", async () => {
    ...
    await waitFor(() => expect(notify).toHaveBeenCalledWith("Haiku saved"));
    expect(HaikuService.updateList).toHaveBeenCalledWith(
      [
  ...
  it("skips the save when the open haiku left the list", async () => {
    ...
    expect(HaikuService.updateList).toHaveBeenCalledTimes(1);
    expect(notify).not.toHaveBeenCalledWith("Haiku saved");
  });
});

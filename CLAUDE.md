...
- Readiness labels: `has-dependencies` marks an issue that needs another
  ...
  Sessions below), never from a label. `next-up` (at most three open
  issues, maintained by the backlog-grooming agent) is a priority hint
  the pipeline works first, not a readiness claim.
